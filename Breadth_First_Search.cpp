...
void BFS_iterative(Graph &graph, int node, int n, vector<int> &ans)
{
    ...
    while (!q.empty())
    {
        ...
        for (int i = 0; i < levelSize; i++)
        {
            ...
            for (int i = 0; i < graph.adjList[node].size(); i++)
            {
                ...
                {
                    visited[graph.adjList[node][i]] = 1;
                    q.push(graph.adjList[node][i]);
                }
            }
        }
            ans.push_back(-1);
...
void BFS_recursive(Graph &graph, int node, vector<int> &visited, vector<int> &ans)
{
    ans.push_back(node);
    visited[node] = 1;
...
int main()
{
    ...
    // BFS_iterative(graph, 1, n, ans);
    BFS_recursive(graph, 1, visited, ans);
